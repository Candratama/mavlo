...
import { getBud, updateBud, deleteBud } from '$lib/server/api/handlers/budgets';
...
export const GET: RequestHandler = async ({ request, params, platform }) => {
	try {
		...
		return ok(await getBud(db, userId, params.id));
	} catch (e) {
...
export const PATCH: RequestHandler = async ({ request, params, platform }) => {
	try {
		...
		return ok(await updateBud(db, userId, params.id, body));
	} catch (e) {
		return toErrorResponse(e);
	}
};
...
export const DELETE: RequestHandler = async ({ request, params, platform }) => {
	try {
		const db = getDb(platform!.env.DB);
		const userId = await requireApiKey(request, db);
		await deleteBud(db, userId, params.id);
		return noContent();
	} catch (e) {
		return toErrorResponse(e);
	}
